...
import com.example.myworkoutapp.databinding.DialogCustomBackConfirmationBinding
import java.util.*
...
class ExerciseActivity : AppCompatActivity(), TextToSpeech.OnInitListener {
    ...
    private fun setupRestView(){
        try {
            val soundURI = Uri.parse("android.resource://com.example.myworkoutapp/" + R.raw.copp)
    ...
    private fun setExerciseProgressBar(){
        binding?.flExerciseView?.progress = exerciseProgress

        exerciseTimer = object: CountDownTimer(exerciseTimerDuration*3000, 1000){
            override fun onTick(p0: Long) {
                exerciseProgress++
                binding?.flExerciseView?.progress = 30 - exerciseProgress
